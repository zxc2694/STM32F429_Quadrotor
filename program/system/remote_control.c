...
void Update_RC_Control(int16_t *Roll, int16_t  *Pitch, int16_t  *Yaw, int16_t  *Thr, int16_t *safety)
{
	...
	if ((system.variable[PWM3_CCR].value > MIN_PWM_INPUT) &&
		...
		*Thr = (PWM_MOTOR_MAX - PWM_MOTOR_MIN) / (MAX_PWM3_INPUT - MIN_PWM3_INPUT) *(system.variable[PWM3_CCR].value - MIN_PWM3_INPUT) + PWM_MOTOR_MIN;
	...
	if ((system.variable[PWM1_CCR].value > MIN_PWM_INPUT) &&
		...
		*Roll = MAX_CTRL_ROLL / (MAX_PWM1_INPUT - MID_PWM1_INPUT) *(system.variable[PWM1_CCR].value - MID_PWM1_INPUT);
#endif

#if USE_DEVO_v10_CONTROLLER
		*Roll = -1*(MAX_CTRL_ROLL / (MAX_PWM1_INPUT - MID_PWM1_INPUT) *(system.variable[PWM1_CCR].value - MID_PWM1_INPUT));
#endif
	}
	...
	if ((system.variable[PWM2_CCR].value > MIN_PWM_INPUT) &&
		...
		*Pitch = MAX_CTRL_PITCH / (MAX_PWM2_INPUT - MID_PWM2_INPUT) *(system.variable[PWM2_CCR].value - MID_PWM2_INPUT);
	}
	...
	if ((system.variable[PWM4_CCR].value > MIN_PWM_INPUT) &&
		...
		*Yaw = MAX_CTRL_YAW / (MAX_PWM4_INPUT - MID_PWM4_INPUT) *(system.variable[PWM4_CCR].value - MID_PWM4_INPUT);
#endif

#if USE_DEVO_v10_CONTROLLER
		*Yaw = -1*(MAX_CTRL_YAW / (MAX_PWM4_INPUT - MID_PWM4_INPUT) *(system.variable[PWM4_CCR].value - MID_PWM4_INPUT));
#endif		
	}
